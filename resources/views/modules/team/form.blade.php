    ...
    <div class='card-body'>
        ...
        <x-form.select name='position_id' label='Position Id'>
            ...
            @foreach($positions as $item)
                <option value='{{ $item->id }}' {{ (old('position_id', $team->position_id ?? '') == $item->id) ? 'selected' : '' }}>{{ $item->name_id }}</option>
            @endforeach
        </x-form.select>
        ...
        <x-form.photo-upload label='Image' name='image' :value="$team->image ?? null" />
