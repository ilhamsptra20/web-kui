    ...
    <div class='card-body'>
        ...
        <x-form.select name='album_id' label='Album Id'>
            ...
            @foreach($albums as $item)
                <option value='{{ $item->id }}' {{ (old('album_id', $gallery->album_id ?? '') == $item->id) ? 'selected' : '' }}>{{ $item->name_id }}</option>
            @endforeach
        </x-form.select>
        <x-form.input name='title_id' label='Title Id' :value="$gallery->title_id ?? ''" floating divider />
        <x-form.photo-upload label='Image' name='image' :value="$gallery->image ?? null" />
